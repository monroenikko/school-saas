...
|------|-------|--------------------------------|
| **`SUPER_ADMIN`** | Platform Root (`tenant_id = null`) | **Controls the whole SaaS platform across all schools.**<br>• Full access across all tenants with `*` permissions.<br>• Manages schools/tenants (provisioning, subscriptions, plans, health).<br>• Platform analytics, global audit logs, system-level settings.<br>• **School Selector / Tenant Switcher in Header**: Can seamlessly toggle between **"🌐 All Schools (Global View)"** and individual schools (e.g. St. Jude, Oakwood, Greenwood). Selecting a school attaches `x-tenant-id` to immediately inspect that specific school's Academic Records (Students, Faculty, Sections, Subjects, Schedules, and Report Cards). |
